...
function t(a, b, c) {
  ...
  for (m = 0; m < n; m++)
    (h =
      a[e >>> 24] ^ q[(f >> 16) & 255] ^ v[(g >> 8) & 255] ^ w[b & 255] ^ d[p]),
      (k =
        a[f >>> 24] ^
        q[(g >> 16) & 255] ^
        v[(b >> 8) & 255] ^
...
sjcl.bitArray = {
  ...
  concat: function (a, b) {
    ...
    return 32 === d
      ? a.concat(b)
      : sjcl.bitArray.$(b, d, c | 0, a.slice(0, a.length - 1));
  },
...
sjcl.codec.base32 = {
  ...
  fromBits: function (a, b, c) {
    ...
    for (c = 0; f.length * d < l; )
      (f += h.charAt((k ^ (a[c] >>> g)) >>> e)),
        g < d ? ((k = a[c] << (d - g)), (g += e), c++) : ((k <<= d), (g -= d));
    for (; f.length & 7 && !b; ) f += '=';
    return f;
  },
  toBits: function (a, b) {
...
sjcl.hash.sha256 = function (a) {
  this.b[0] || this.O();
  a
    ? ((this.F = a.F.slice(0)), (this.A = a.A.slice(0)), (this.l = a.l))
    : this.reset();
};
...
sjcl.prng.prototype = {
  randomWords: function (a, b) {
    ...
    if (d & this.J) {
      ...
      for (
        d = 0;
        4 > d && ((this.h[d] = (this.h[d] + 1) | 0), !this.h[d]);
        d++
      );
    }
    for (d = 0; d < a; d += 4)
